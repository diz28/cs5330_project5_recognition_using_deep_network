...
from main import get_data, train


# network model for experimental use
class Net(nn.Module):
...
# testing method for experimental purposes - testing different variations of the network
    ...
    for i in range(len(activation_fn_list)):
        for j in range(len(dropout_rate_list)):
            for k in range(len(training_data_size)):
                ...
                train_loader, test_loader = get_data(batch_size_train, batch_size_test, training_data_size[k])
